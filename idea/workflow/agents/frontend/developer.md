...
If the session task points to a folder, process all files inside the folder step-by-step (sorted order), running the full implementation workflow for each file individually. If the session task points to a single file, proceed as usual.
...
1. Read task context from the session and use it to understand the scope of work:
   ...
   c. If the `task.link` or `task` points to a folder:
   - List all files in the folder (non-recursive), sorted (e.g., numerically/alphabetically):
     ```bash
     TASK_FILES=( $(ls -1 "$TASK_PATH" | sort) )
     ```
   - For each file in `TASK_FILES`, repeat steps 2–4 below for that file, treating each as an individual task (extract title/description from the file as needed).
     d. If the `task.link` or `task` points to a file, proceed as below.
