...
d. Use the **trev-gitlab-workflow** skill to **poll** the pipeline for the MR branch every **30 seconds** until it reaches a terminal state (`passed`, `success`, `failed`, `canceled`, `skipped`). Do **not** proceed to Step 4 while the pipeline is still `running` or `pending`. **Ignore any polling timeout if the pipeline has already reached a terminal state** — a finished pipeline always takes priority over a timeout error.
