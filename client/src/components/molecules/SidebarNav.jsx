...
const SidebarNav = ({ collapsed }) => (
  <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3" role="navigation" aria-label="Primary">
    {navSections.map((section, idx) => (
      <div className="mb-4" key={idx}>
        <div className={`text-[10px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-600 px-4 py-2 whitespace-nowrap overflow-hidden transition-all duration-200 ${collapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'}`}>{section.label}</div>
        {section.items.map((item) => (
          ...
          >
            ...
            <span className={`text-sm font-medium overflow-hidden transition-all duration-200 ${collapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'}`}>{item.label}</span>
            {item.badge && <span className={`ml-auto bg-rose-500 text-white text-[10px] font-bold leading-none px-2 py-0.5 rounded-full flex-shrink-0 transition-all duration-200 ${collapsed ? 'opacity-0 w-0 p-0' : 'opacity-100 w-auto'}`}>{item.badge}</span>}
          </NavLink>
        ))}
      </div>
    ))}
