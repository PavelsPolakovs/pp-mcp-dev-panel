...
export default function CommandButton({ tool, label, description, icon: Icon, colorClass }) {
  ...
  return (
    <button
      ...
      `}
    >
      <div className={`flex-shrink-0 ${isRunning ? "text-cyan-500 dark:text-cyan-400" : isDisabled ? "text-zinc-400 dark:text-zinc-700" : colorClass}`}>
        {isRunning
      ...
      <div className="text-left flex-1 min-w-0">
        <div className="text-sm font-medium leading-tight text-zinc-900 dark:text-zinc-100">{label}</div>
        <div className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5 font-mono truncate">{description}</div>
      </div>
      {isRunning && (
        <span className="text-xs text-cyan-700 dark:text-cyan-500 animate-pulse flex-shrink-0">running…</span>
      )}
    </button>
  );
}
